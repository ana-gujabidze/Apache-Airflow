...
class TestDAG3(unittest.TestCase):
    ...
    def setUpClass(self):
        self.dagbag = DagBag()
        self.dag = self.dagbag.get_dag(dag_id='my_dag3')
    ...
    def test_count_and_contain_tasks(self):
        ...
        self.assertListEqual(task_ids, ['get_time', 'slack', 'bash_task'])
    ...
    def test_dependencies_of_slack(self):
        ...
        downstream_task_ids = [task.task_id for task in task.downstream_list]
        self.assertListEqual(downstream_task_ids, ['bash_task', ])

    def test_dependencies_of_bash_task(self):
        """
        Test checks dependencies of dag with id bash_task.
        """
        task_id = 'bash_task'
        task = self.dag.get_task(task_id)

        upstream_task_ids = [task.task_id for task in task.upstream_list]
        self.assertListEqual(upstream_task_ids, ['slack', ])
        downstream_task_ids = [task.task_id for task in task.downstream_list]
    ...
    def test_find_time(self):
        ...
        '''
        with freeze_time("2021-05-20 15:15"):
            result = find_time_v_1()
            # 19 hour because of time difference in find_time() function.
            self.assertEqual(result, 'It is: 19:15')
...
suite = unittest.TestLoader().loadTestsFromTestCase(TestDAG3)
